...
def measure_dirs(dirA, dirB, use_gpu, verbose=False):
    ...
    with open('Report.txt', 'a') as f:
        f.writelines("**************** METHOD: INTER_LANCZOS4***************\n")
        f.writelines("Origin\tUpscaled\t\t psnr || ssim || lpips || time\n")
    ...
    for pathA, pathB in zip(paths_A, paths_B):
        ...
        d = time.time() - t
        with open('Report.txt', 'a') as f:
            f.writelines(f"{pathA.split('/')[-1]} , {pathB.split('/')[-1]}\t, {format_result(**result)}\t, {d:0.1f}\n")
    ...
    with open('Report.txt', 'a') as f:
        f.writelines(f"Final Result\t\t: {format_result(psnr, ssim, lpips)}, {time.time() - t_init:0.1f}s\n\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument('-dirA', default='c_i', type=str)
    parser.add_argument('-dirB', default='c_o', type=str)
